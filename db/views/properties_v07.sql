WITH
  lms_properties AS (
    ...
    FROM
      ...
      JOIN regions r ON ST_Intersects(zp.location, r.area) AND r.title = 'Southern PEC'
    ),
  purple_bricks_properties AS (
    ...
    FROM
      ...
      JOIN regions r ON ST_Intersects(pbp.location, r.area) AND r.title = 'Southern PEC'
  ),
  prelist_properties AS (
    ...
    FROM
      ...
      JOIN regions r ON ST_Intersects(p.location, r.area) AND r.title = 'Southern PEC'
  ),
  properties AS (
    SELECT *
    FROM lms_properties
