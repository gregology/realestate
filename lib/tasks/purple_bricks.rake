...
task :extract_purple_bricks_properties => :environment do
...
task :update_purple_bricks_properties => :environment do
  include ApplicationHelper

  purple_bricks_properties = PurpleBricksProperty.where(address: nil).limit(rand(17..23))
